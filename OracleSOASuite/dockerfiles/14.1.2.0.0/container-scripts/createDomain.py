...
class SOA12214Provisioner:
    ...
    def reConfigureJMSStore(self, domainHome, domainType):
        readDomain(domainHome)
        print 'Read Domain: %s' % domainHome
        configUpdated = false
        serverList = []
        if domainType == "soa" or domainType == "soaosb" or domainType == "soab2b" or domainType == "soaosbb2b":
            serverList.extend(list(self.SOA_SERVERS.keys()))
        ...
        if dataSource == None:
            print 'WLSSchemaDataSource not exists in this domain, skipping'
            closeDomain()
            return
        schemaPrefix = self.getDSSchemaPrefix ('WLSSchemaDataSource')
        cmd = cd('/')
        ...
        for jmsServer in jmsServers:
            ...
            if (jmsFlag == true):
                ...
                print '[JMSServer]: ' + str(jmsServerName)+ ' : [TargetServer]: ' + str(jmsServerTargetName)
                ...
                print 'CONTINUE processing ... ['+jmsServerName+\
                              '] will not be configured to use JDBC Persistent Store'
                continue
        print 'clean UnUsedSOAStores from Domain : '+ domainHome
        configUpdated= configUpdated | self.cleanunUsedSOAStores()
        if configUpdated:
            print 'Updating Domain : ',domainHome
            updateDomain()
        else:
            print 'No Updates Closing Domain with out update ',domainHome
        closeDomain()
        print 'COMPLETED jms jdbc store config '
        ...
        if(jmsJdbcStoreName == None):
            ...
            if(jmsJdbcStoreName != oldPersistence):
                try:
                    ...
                    dumpStack()
                    print 'setPersistentStore failed for jmsServerName '+str(jmsServerName), detail , " !!!!!! If the exception above says 'com.oracle.cie.domain.ValidateException: The property value is duplicated'; means you did not apply CIE fix of bug 22175233 "     
                    raise
    ...
    def getJMSJDBCStore(self, jmsServerName,jmsServerTargetName, schemaPrefix, dataSource):
        try: 
            ...
            if (get('PersistentStore') != None):
                ...
                return None
            print 'Parsing PersistentStore: ' +str(originalStore) +" jmsServerTargetName "+str(jmsServerTargetName)
            ...
            if (store == None):
                ...
                print 'INFO: jdbcStore ' , store.getTargets() , ' Expected ', jmsServerTargetName
    ...
    def targetJMSJDBCStore(self, jStore, jmsServerName, jmsServerTargetName, schemaPrefix, dataSource):
        ...
        jmsTargets = jarray.array([jmsTarget], Class.forName("weblogic.management.configuration.TargetMBean"))
        print 'INFO: current Target ',cmo.getTargets(),' Expected Target ', jmsTarget
        cmo.setTargets(jmsTargets)
